...
# =============================================================================
# ARCHIVED -- PRE-VIM3 SNAPSHOT (2026-07-24)
# This file reflects TAPP terminology before alignment with BIPM VIM3
# (JCGM 200:2012): "Protocol" = the registerable, DOI-bearing object;
# "Procedure"/"Analysis" = the analysis-level execution. Kept for historical
# record only -- do not use for current TAPP work. Current version lives at
# the original path (same filename, without this suffix).
# =============================================================================
...
Column structure (columns A–H are fixed; I onward is dynamic):
  ...
  I+n+2 …  Literature assessment columns (one per extracted protocol)
...
# ---------------------------------------------------------------------------
TIER_STYLES = {
    # Protocol-Level Tier (col C) and Analysis-Level Tier (col D)
...
COL_WIDTHS = {
    ...
    2: 68,   # B — Description
    3: 15,   # C — Protocol-Level Tier
    4: 15,   # D — Analysis-Level Tier
    5: 20,   # E — Data Type
    6: 48,   # F — Example/Allowed Content
    7: 22,   # G — Comments
    8: 14,   # H — Last update
    # I onward: set dynamically below
}
MODE_COL_WIDTH = 13
LIT_COL_WIDTH  = 30
...
# Build Legends sheet
# ---------------------------------------------------------------------------
def build_legends(wb, mode_headers):
    ...
    r = 1
    hdr(r, 1, 'Protocol-Level Tier'); hdr(r, 2, 'Description')
    hdr(r, 3, 'Analysis-Level Tier'); hdr(r, 4, 'Description')
    r += 1

    proto_tiers = [
        ('Basic',    'C00000', 'Mandatory for protocol registration. Must be provided to register a valid protocol.'),
        ('Advanced', '375623', 'Optional for protocol registration. Strongly recommended but not required.'),
        ('N/A',      '000000', 'Not applicable at protocol level. This field captures analysis-level information only.'),
    ]
    analysis_tiers = [
        ('Read-Only', '0070C0', 'Directly imported from the registered protocol; cannot be changed by the analyst. Changing this value means running a different protocol. Also used for fields relevant only at protocol level — the value is inherited from the protocol record and shown read-only in the analysis form.'),
        ('Editable',  '7030A0', 'Imported from the registered protocol but may be adjusted within protocol-defined bounds (e.g., daily tuning, minor software updates). The protocol registers the target or typical value; the analyst confirms or adjusts it. Cannot be left void if the protocol-level tier is Basic.'),
        ('Basic',     'C00000', 'Mandatory user input at analysis time. Value comes from the analysis (procedure) itself and cannot be pre-specified in the protocol.'),
...
def convert(csv_path, xlsx_path):
    ...
    for ri, row in enumerate(rows, start=1):
        ...
        for ci, val in enumerate(row, start=1):
            ...
            # Protocol-Level Tier (col C = index 2)

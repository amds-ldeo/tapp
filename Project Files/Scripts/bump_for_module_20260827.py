...
"""Recompose and bump every consumer of a module, stamping Last Update on the rows it owns.
  ...
  python3 bump_for_module.py <ModuleName> "<Source: ... module>" [--apply]

Composition writes straight to the NEW version path so each superseded copy is the file as it was
published. Refuses to write unless every named field carries the module's provenance stamp after
composition.
"""
...
def main(module,stamp,apply=False):
    modrows=list(csv.reader(open(os.path.join(ROOT,"Claude Skills for TAPP","modules","Module_%s.csv"%module),
                                 newline="",encoding="utf-8-sig")))
    FIELDS={r[0].strip() for r in modrows[1:] if r and r[0].strip()}
    reg=json.load(open(os.path.join(ROOT,"composed_tapps.json"),encoding="utf-8"))
    targets=[e for e in reg["composed"] if any(m["name"]==module for m in e["modules"])]
    plan=[]
    for e in targets:
    ...
    for e,rel,new in plan:
        ...
        for r in rows[1:]:
            if r and r[0].strip() in FIELDS:
                ...
                if r[ic].strip()!=stamp: raise SystemExit("%s: %s missing stamp (got %r)"%(new,r[0],r[ic]))
                r[iu]=DATE; n+=1
        if n!=len(FIELDS): raise SystemExit("%s: stamped %d of %d"%(new,n,len(FIELDS)))

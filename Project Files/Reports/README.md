...
| `EPMA_TAPP_v77_Review_Workbook_<Mode>.xlsx` (4 files) | one workbook per analytical mode: EDS Point Analysis (73 items), EDS Mapping (76), WDS Point Analysis (78) and WDS Mapping (78). Each shows only the items flagged Y for its mode, with no mode columns and TAPP item numbers kept. All five workbooks carry an Example column taken from the worked example, filtered per mode to the elements and conditions that mode uses. |

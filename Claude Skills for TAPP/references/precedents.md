...
## Literature attestation cannot validate lab-internal fields (2026-09-16)

**Decision: `Session Identifier` is not given a Phase 3 pass, and its 131 in-situ literature cells are
left blank on purpose.** The finding generalises to a class of fields, which is why it is a precedent
rather than a note in one dev-log entry.

**The check.** Of the columns already assessed, 46 of 46 are `N` — Solution (29), Lab-XCT (14), and the
single filled EPMA, LA-MC and LA-Q columns. Those are disjoint branches: solution chemistry and X-ray
tomography share no methods, no instruments and no authors, and they agree. Across the 49 in-situ
papers, 14 mention a session at all and every one is scheduling or QC context — *"measured at the start
and end of each analytical session"*, *"a NIST 610 glass standard was analyzed before and after every
session"* — never an identifier. No paper dates its analyses in the ordinary way either.

**The two near-misses, recorded so they are not re-derived.** Barnes et al. 2025 labels *"(session 1)"*
and *"(session 2)"*, but those name two element-suite configurations, not runs. Singerling et al. 2025
identifies *"the four TEM sessions (December 2023, January 2024, May 2024, and December 2024)"* — a
genuine distinction between sessions, but by date, and only because the paper's subject is how the
carbonates changed between them. Neither is *"the laboratory's own run, sequence or batch identifier as
generated by the instrument or acquisition software"*.

**Why this is structural.** Every literature column is a PROCEDURE extraction. An analysis-level field
survives that only when the value is something a paper prints. Sorting the analysis-level-only fields by
how often the literature states them separates them cleanly into two kinds:

| Field | stated | `N` |
|---|---|---|
| `Sampling Unit Name` | 142 | 35 |
| `Sample Name` | 92 | 85 |
| `Analyst` | 19 | 158 |
| `Analysis Start Date` | 2 | 163 |
| `Procedure DOI` | 0 | 165 |
| `Session Identifier` | **0** | 46 |

The top of the table is what papers publish; the bottom is what laboratories and repositories keep to
themselves. `Session Identifier` sits at the bottom with `Procedure DOI` and the date fields.

**What this does NOT license.** Literature silence is not evidence a field is unused or unneeded. This
field's purpose — the link back to the raw instrument files — is a submission-time need, and the
corpus is simply the wrong instrument for testing it, exactly as it is for `Analyst` and the date
fields. The rule: **absence of literature attestation falsifies a field only when the value is the

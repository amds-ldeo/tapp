...
`paper_registry.csv` lives at `/Users/ruolin/Documents/Astromat/TAPPs/Project Files/Registers & Planning/paper_registry.csv`.
...
Technique columns use the **exact "Proposed TAPP Name"** from `Project Files/Registers & Planning/TAPP_Planning_Table.csv`.
...
If a technique used in a paper does not appear in `Project Files/Registers & Planning/TAPP_Planning_Table.csv`, record it in the `N — <note>` format in the closest matching column (or add a temporary "Other" note) and flag it to the user immediately so the planning table can be updated.
